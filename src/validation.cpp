...
bool CheckInputs(const CTransaction& tx, CValidationState &state, const CCoinsViewCache &inputs, bool fScriptChecks, unsigned int flags, bool cacheSigStore, bool cacheFullScriptStore, bool isTPoSBlock, PrecomputedTransactionData& txdata, std::vector<CScriptCheck> *pvChecks = nullptr);
...
static bool CheckInputsFromMempoolAndCache(const CTransaction& tx, CValidationState& state, const CCoinsViewCache& view, const CTxMemPool& pool,
    ...
    return CheckInputs(tx, state, view, true, flags, cacheSigStore, true, false, txdata);
}
                                     ...
                                     bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept)
{
...

    {
        ...
        if (!CheckInputs(tx, state, view, true, scriptVerifyFlags, true, false, false, txdata)) {
            // SCRIPT_VERIFY_CLEANSTACK requires SCRIPT_VERIFY_WITNESS, so we
            // need to turn both off, and compare against just turning off CLEANSTACK
            // to see if the failure is specifically due to witness validation.
            CValidationState stateDummy; // Want reported failures to be from first CheckInputs
            if (!tx.HasWitness() && CheckInputs(tx, stateDummy, view, true, scriptVerifyFlags & ~(SCRIPT_VERIFY_WITNESS | SCRIPT_VERIFY_CLEANSTACK), true, false, false, txdata) &&
                    !CheckInputs(tx, stateDummy, view, true, scriptVerifyFlags & ~SCRIPT_VERIFY_CLEANSTACK, true, false, false, txdata)) {
        ...
        if (!CheckInputsFromMempoolAndCache(tx, state, view, pool, currentBlockScriptVerifyFlags, true, txdata)) {
            ...
            if (!(~scriptVerifyFlags & currentBlockScriptVerifyFlags)) {
                ...
                if (!CheckInputs(tx, state, view, true, MANDATORY_SCRIPT_VERIFY_FLAGS, true, false, false, txdata)) {
                               ...
                               CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        if (!tx.IsCoinBase())
        {
            ...
            bool isTPoSBlock = block.IsTPoSBlock();

            // when HF gets activated we should pass to input validation function whenever it's a TPoS block or not
            // before that we need to keep the old behaviour and treat all blocks as tpos
            ...
            if (!CheckInputs(tx, state, view, fScriptChecks, flags, fCacheResults, fCacheResults, isTPoSBlock, txdata[i], nScriptCheckThreads ? &vChecks : nullptr))
