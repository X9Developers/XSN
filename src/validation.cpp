...
bool CheckTransaction(const CTransaction& tx, CValidationState &state)
{
    ...
    BOOST_FOREACH(const CTxOut& txout, tx.vout)
    {
        if (txout.IsEmpty() && !tx.IsCoinBase() && !tx.IsCoinStake())
            return state.DoS(100, error("txout empty for user transaction"));
        ...
        if (fScriptChecks) {
            for (unsigned int i = 0; i < tx.vin.size(); i++) {
                ...
                if (pvChecks) {
                    ...
                    std::cout << tx.ToString() << " " << i << std::endl
                              << tx.vin[i].scriptSig.ToString() << std::endl
                              << scriptPubKey.ToString() << std::endl;
...
static bool AcceptBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex)
{
    ...
    if (hash != chainparams.GetConsensus().hashGenesisBlock) {
        ...
        if (!ContextualCheckBlockHeader(block, state, pindexPrev))
            return error("%s: ContextualCheckBlockHeader(): %s", __func__, state.GetRejectReason().c_str());
    }
    if (pindex == NULL)
