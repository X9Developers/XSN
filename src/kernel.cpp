    ...
    *pindexSelected = (const CBlockIndex*)0;
    for (const auto & item : vSortedByTimestamp) {
        ...
        if(fModifierV2)
            ...
            hashProof = pindex->IsProofOfStake() ? ArithToUint256(0) : pindex->GetBlockHash();
                          ...
                          bool fTPoS)
{
    ...
    if (!fTPoS && nTimeBlockFrom + nStakeMinAge > nTimeTx) // Min age requirement- will always pass in tpos
    ...
    CAmount nValue = nValueIn;

    //    if(fTPoS)
    //        nValue *= 10; // modify temporary kernel hash
...
bool CheckProofOfStake(CWallet *wallet, const CBlock block, uint256& hashProofOfStake)
{
    ...
    if (!CheckStakeKernelHash(block.nBits, blockprev, txPrev, txin.prevout, nTime, nInterval, true, hashProofOfStake, fDebug, false/*block.IsTProofOfStake()*/))
