This directory contains the XSNQT graphical user interface (GUI). It uses the cross-platform framework [Qt](https://www1.qt.io/developers/).

The current precise version for Qt 5 is specified in [qt.mk](/depends/packages/qt.mk). Qt 4 is also supported (see [#8263](https://github.com/xsn/xsn/issues/8263)).
...
Represents the main window of the XSN UI.
...
ClientModel is used by the main application `xsngui` and several models like `peertablemodel`.
...
Used to process BIP21 and BIP70 (see https://github.com/xsn/xsn/pull/11622) payment URI / requests. Also handles URI based application switching (e.g. when following a xsn:... link from a browser).
...
* UI elements like XSNAmountField, which inherit from QWidget.
* `xsnstrings.cpp`: automatically generated
* `xsnunits.(h/cpp)`: BTC / mBTC / etc handling
* `callback.h`
...
* don't change `local/xsn_en.ts`; this happens [automatically](/doc/translation_process.md#writing-code-with-translations)
...
10. Start debugging with Qt Creator (you might need to the executable to "xsn-qt" under "Run", which is where you can also add command line arguments)
