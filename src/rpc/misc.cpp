...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total xsn balance of the wallet\n"
            "  \"privatesend_balance\": xxxxxx, (numeric) the anonymized xsn balance of the wallet\n"
...
UniValue debug(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "xsn|privatesend|instantsend|masternode|spork|keepass|mnpayments|gobject )\n"
            "Change debug category on the fly. Specify single category or use comma to specify many.\n"
            "\nExamples:\n"
            + HelpExampleCli("debug", "xsn")
            + HelpExampleRpc("debug", "xsn,net")
        );
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"xsnaddress\"\n"
            "\nReturn information about the given xsn address.\n"
            "\nArguments:\n"
            "1. \"xsnaddress\"     (string, required) The xsn address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"xsnaddress\", (string) The xsn address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: XSN address and we have full public key:
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
    {
        string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are xsn addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) xsn address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"xsnaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"xsnaddress\"  (string, required) The xsn address to use for the signature.\n"
...
UniValue checkposblock(const UniValue& params, bool fHelp)
{
    ...
    if (it != mapBlockIndex.end())
        ...
        LogPrintf("CheckProofOfStake() : read block failed");

    // Read block header
    CBlock blockprev;
    if (!ReadBlockFromDisk(blockprev, pindex->GetBlockPos(), cons))
        LogPrintf("CheckProofOfStake(): INFO: failed to find block");
