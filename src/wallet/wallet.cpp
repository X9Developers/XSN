...
bool CWallet::AddToWalletIfTPoSContract(const CTransaction &tx, const CBlock *pblock)
{
    AssertLockHeld(cs_wallet);
...
void CWallet::SyncTransaction(const CTransaction& tx, const CBlock* pblock)
{
    ...
    AddToWalletIfTPoSContract(tx, pblock);
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                if (!(IsSpent(wtxid, i)) && mine != ISMINE_NO &&
                        (!IsLockedCoin((*it).first, i) || nCoinType == ONLY_MASTERNODE_COLLATERAL || nCoinType == ONLY_MERCHANTNODE_COLLATERAL) &&
    ...
    for (auto& outpoint : setWalletUTXO) {
        ...
        for (unsigned int i = 0; i < wtx.vout.size(); i++) {
            ...
            if(fAnonymizable) {
                ...
                if(fMasterNode && wtx.vout[i].nValue == 15000 * COIN) continue;
...
bool CWallet::GetMerchantnodeOutpointAndKeys(COutPoint &outpointRet, CPubKey &pubKeyRet, CKey &keyRet, string strTxHash, string strOutputIndex)
{
    // wait for reindex and/or import to finish
    if (fImporting || fReindex) return false;
    ...
    // Find possible candidates
    std::vector<COutput> vPossibleCoins;
    AvailableCoins(vPossibleCoins, true, NULL, false, ONLY_MERCHANTNODE_COLLATERAL);
    if(vPossibleCoins.empty()) {
        LogPrintf("CWallet::GetMerchantnodeOutpointAndKeys -- Could not locate any valid masternode vin\n");
        return false;
    }
    ...
    LogPrintf("CWallet::GetMerchantnodeOutpointAndKeys -- Could not locate specified masternode vin\n");
    return false;
}
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                if (fUseInstantSend && nValueIn > sporkManager.GetSporkValue(SPORK_5_INSTANTSEND_MAX_VALUE)*COIN) {
                    strFailReason += " " + strprintf(_("InstantSend doesn't support sending values that high yet. Transactions are currently limited to %1 XSN."), sporkManager.GetSporkValue(SPORK_5_INSTANTSEND_MAX_VALUE));
                    return false;
                }
...
bool CWallet::RemoveTPoSContract(const uint256 &contractTxId)
{
    ...
    if (fFileBacked)
        if (!CWalletDB(strWalletFile).EraseTPoSContractTx(contractTxId))
            return false;
