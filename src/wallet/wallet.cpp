...
bool CWallet::IsDenominated(const COutPoint& outpoint) const
{
    ...
    auto mi = mapWallet.find(outpoint.hash);
    if (mi != mapWallet.end()) {
                              ...
                              std::vector<const CWalletTx*> &vwtxPrev)
{
    ...
    for(const std::pair<const CWalletTx*, unsigned int> &pcoin : setStakeCoins)
    {
        ...
        if (it != mapBlockIndex.end())
            ...
            LogPrint(BCLog::KERNEL, "failed to find block index \n");
            continue;
        }
...
bool CWallet::GetMasternodeOutpointAndKeys(COutPoint& outpointRet, CPubKey& pubKeyRet, CKey& keyRet, std::string strTxHash, std::string strOutputIndex)
{
    ...
    AvailableCoins(vPossibleCoins, true, nullptr, 1, MAX_MONEY, MAX_MONEY, 0, 0, 9999999, ONLY_MASTERNODE_COLLATERAL);
