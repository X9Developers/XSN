...
UniValue sendtoaddress(const UniValue& params, bool fHelp)
{
    ...
    SendMoney(address.Get(), nAmount, fSubtractFeeFromAmount, amountOfSplits, wtx, fUseInstantSend, fUsePrivateSend);
...
UniValue sendmany(const UniValue& params, bool fHelp)
{
    ...
    BOOST_FOREACH(const string& name_, keys)
    {
        ...
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid XSN address: ")+name_);
...
UniValue encryptwallet(const UniValue& params, bool fHelp)
{
    ...
    StartShutdown();
    return "Wallet encrypted; XSN Core server stopping, restart to run with encrypted wallet. The keypool has been flushed and a new HD seed was generated (if you are using HD). You need to make a new backup.";
}
