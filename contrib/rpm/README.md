...
The RPM spec file provided here is for XSN-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://xsn.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[XSN git repository](https://github.com/xsn/xsn) but are not part of
...
Currently only `Source10` is used, for the example `xsn.conf` file.
...
in the xsn git repository. At present only an SVG version of the XSN
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the xsn
...
needed when 0.12.1 is released, a proper fix is already in the XSN git
master branch.
...
idea to build XSN Core against a static version of that library compiled
...
The `xsn.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `xsn-server` package. At this point, most RPM based
...
The `xsn-server` package should have SELinux support. How to properly do
...
XSN-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/xsn/). He has been
packaging XSN for Fedora at least since 2012.
...
4. I renamed the `xsn` package that contains the Qt GUI to `xsn-core` as
that appears to be how the general population refers to it, in contrast to
`xsn-xt` or `xsn-classic`. I wanted to make sure the general population
